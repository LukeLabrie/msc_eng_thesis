...
beta_vec = [0.000223,0.001457,0.001307,0.002628,0.000766,0.00023]; % U235
% beta   = [0.00023,0.00079,0.00067,0.00073,0.00013,0.00009]; % U233
beta_t = sum(beta_vec); % total delayed neutron fraction MSRE
rho_0  = beta_t - bigterm(beta_vec,lam,tau_l,tau_c); % reactivity change in going from stationary to circulating fuel
C0(1)  = ((beta_vec(1))/Lam)*(1.0/(lam(1) - (exp(-lam(1)*tau_l) - 1.0)/tau_c));
C0(2)  = ((beta_vec(2))/Lam)*(1.0/(lam(2) - (exp(-lam(2)*tau_l) - 1.0)/tau_c));
C0(3)  = ((beta_vec(3))/Lam)*(1.0/(lam(3) - (exp(-lam(3)*tau_l) - 1.0)/tau_c));
C0(4)  = ((beta_vec(4))/Lam)*(1.0/(lam(4) - (exp(-lam(4)*tau_l) - 1.0)/tau_c));
C0(5)  = ((beta_vec(5))/Lam)*(1.0/(lam(5) - (exp(-lam(5)*tau_l) - 1.0)/tau_c));
C0(6)  = ((beta_vec(6))/Lam)*(1.0/(lam(6) - (exp(-lam(6)*tau_l) - 1.0)/tau_c));
