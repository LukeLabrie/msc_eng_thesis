...
def sumSq_ht_all(params):
    try:
        ...
        fuel_in_error = sum((fuel_in-F_to_K(1209)))

        fuel_out = np.array([s[1] for s in sol_jit])[i_eq]
        fuel_out_error = sum((fuel_out-F_to_K(1522)))

        # this should force coolant temperatures to agree

        tot = pow_error + fuel_in_error + fuel_out_error
