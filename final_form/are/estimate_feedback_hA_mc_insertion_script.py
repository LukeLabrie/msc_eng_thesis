...
spline = CubicSpline(df[0], df[1]) 
interpolated_values = spline(T[i_insert[0]:i_insert[-1]+1])

spline_inlet = CubicSpline(df_inlet[0], df_inlet[1]) 
interpolated_inlet = spline(T[i_insert[0]:i_insert[-1]+1])

spline_outlet = CubicSpline(df_outlet[0], df_outlet[1]) 
interpolated_outlet = spline(T[i_insert[0]:i_insert[-1]+1])
...
def sumSq_initial(params):
    try:
        ...
        # calculate error
        simulation_output = np.array([s[6]*P for s in sol_jit])[i_insert]
        error = sum((simulation_output - interpolated_values)**2)  # Sum of squared errors
        return error
...
def sumSq_initial_wm(params):
    try:
        # run
        sol_jit = relax_feedback_wm(params)

        # calculate error
        simulation_output = np.array([s[6]*P for s in sol_jit])[i_insert]
        error = sum((simulation_output - interpolated_values)**2)  # Sum of squared errors

        fuel_in = np.array([s[15] for s in sol_jit])[i_insert]
        fuel_in_error = sum((fuel_in-interpolated_inlet)**2)

        fuel_out = np.array([s[1] for s in sol_jit])[i_insert]
        fuel_out_error = sum((fuel_out-interpolated_outlet)**2)

        return error + fuel_in_error + fuel_out_error
...
def estimate_feedback():
    ...
    hA_mc_c0 = hA_mc_c
    hA_mc_bounds = (hA_mc_c/20,20*hA_mc_c)

    initial_guess = [a_f0,a_b0,a_c0,hA_mc_c0]
    bounds = [a_f_bounds,a_b_bounds,a_c_bounds,hA_mc_bounds]

    # minimize
    result = minimize(sumSq_initial_wm, initial_guess, bounds=bounds,method='Nelder-Mead')
    return result
...
def main():
    ...
    file_path = "output_feedback_insertion_power_wm.txt"
