...
# data from manual python implementation
sol = []
...
# data from matlab
df_simulink = pd.read_excel("../data/k_150_simulink.xlsx")
df_dde23 = pd.read_excel("../data/sim_out_150.xlsx")

plt.plot(T,[r[12] for r in data_solver],label="jitcdde")
plt.plot([float(s[0]) for s in sol],[float(s[13]) for s in sol],label="luke's solver")
plt.plot(df_simulink['time'],df_simulink['Mux(4)'],label="simulink")
plt.plot(df_dde23[0],df_dde23[1],label="dde23")
plt.title(r'$n$')
plt.legend()
plt.xlabel('t (s)')
